<#
.SYNOPSIS
    Installs the CLEVR Lint Studio Pro extension into a Mendix project.

.DESCRIPTION
    Copies the bundled 'clevrlint' extension folder into <your Mendix project>\extensions\clevrlint,
    detects mxcli on your PATH, and writes a clean lint-scan-settings.json for your project.
    Run this from the folder where it lives (next to the 'clevrlint' folder).
...
function Install-Mxcli {
    ...
    try {
        ...
        $headers = @{ 'User-Agent' = 'clevr-lint-installer'; 'Accept' = 'application/vnd.github+json' }
        ...
        Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $tmp -Headers @{ 'User-Agent' = 'clevr-lint-installer' } -TimeoutSec 600
...
Write-Host ""
Write-Host "CLEVR Lint - extension installer" -ForegroundColor Cyan
Write-Host "Requires Mendix Studio Pro 11 or higher." -ForegroundColor DarkYellow
Write-Host ""

# The 'clevrlint' folder must sit next to this script.
$sourceDir = Join-Path $PSScriptRoot 'clevrlint'
if (-not (Test-Path $sourceDir)) {
    Write-Error "Could not find the 'clevrlint' folder next to this script ($sourceDir). Keep the script in the package folder."
    exit 1
}
...
$mxcliCacheDir = Join-Path $env:LOCALAPPDATA 'clevr-lint\mxcli'
...
# 3) Offer to download (explicit confirmation - never a silent internet download).
    ...
    Write-Host "  The CLEVR Lint scan needs mxcli (the official Mendix Labs release binary)." -ForegroundColor Yellow
    ...
    if (-not $mxcliPath) {
        ...
        Write-Host "    2. Put it on your PATH, or set its full path in lint-scan-settings.json (mxcliPath)." -ForegroundColor Yellow
...
# --- Target: <project>\extensions\clevrlint ----------------------------------------------------
$targetDir = Join-Path (Join-Path $projectDir 'extensions') 'clevrlint'

# On upgrade: preserve existing, working settings values (don't overwrite without need).
$existing = $null
$settingsFile = Join-Path $targetDir 'lint-scan-settings.json'
...
Write-Host "  4. Open the panel via the Extensions menu > 'CLEVR Lint' and click Scan."
if (-not $mxcliPath) {
