...
export function ViolationInstance({ rule, v, interactive = true }: Props) {
  ...
  return (
    <div className="acr-instance">
      {v.kind === "manual" && v.manual ? (
        <div className="acr-manual-state">{manualStateLabel(v.manual)}</div>
      ) : (
        <div
          className={"acr-doc" + (interactive ? " acr-doc-clickable" : "")}
        ...
        >
          ...
          {interactive && <span className="acr-open-hint"> ↗ open</span>}
        </div>
      )}
      <div className="acr-reason">{v.reason}</div>
      {v.suggestion && <div className="acr-suggestion">{v.suggestion}</div>}
      {interactive && (
        <div className="acr-instance-actions">
          {v.kind === "manual" && v.manual && (
            <button type="button" className="acr-answer-btn" onClick={() => setShowManual(true)}>
          ...
          <button type="button" className="acr-exclude-btn" title="Exclude this improvement (a reason is required)" onClick={() => setShowExclude(true)}>
            Exclude
          </button>
        </div>
      )}
      {v.documentationUrl && (
        <div className="acr-docslink">
          <a
