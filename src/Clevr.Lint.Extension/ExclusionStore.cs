...
/// <summary>
/// Reading/writing of <c>$project/.clevr-lint/exclusions.json</c> (spec section 3). The pure
...
public sealed class ExclusionStore
{
    ...
    public void Add(string? projectDir, Exclusion exclusion)
    {
        if (string.IsNullOrWhiteSpace(projectDir))
            throw new InvalidOperationException("No project folder available to save the exclusion.");
    ...
    {
        if (string.IsNullOrWhiteSpace(projectDir))
            throw new InvalidOperationException("No project folder available to save the exclusions.");
        var list = Load(projectDir);
    ...
    {
        if (string.IsNullOrWhiteSpace(projectDir))
            throw new InvalidOperationException("No project folder available to remove the exclusion.");
    ...
    {
        if (string.IsNullOrWhiteSpace(projectDir))
            throw new InvalidOperationException("No project folder available to remove the exclusions.");
