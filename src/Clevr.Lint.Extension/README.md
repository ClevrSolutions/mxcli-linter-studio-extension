# CLEVR Lint — C# spike: process execution + message passing

Targeted spike for the open assumption from
[`../clevr-lint-shell-spec.md`](../clevr-lint-shell-spec.md) **section 7 / point 4**:
...
## Phase 2A + 2B — real mxcli data in the Lint layout (built on this spike)

On top of the proven spike (Process.Start + message passing), the **real mxcli engine**
has been connected to the **existing, tested normalizer** (part A), and the **Lint layout**
...
pane → **Lint layout** (`wwwroot/main.js`). The `engine` property is **not** shown.

**Lint layout (part B), as per spec section 5:**
...
- **Everything in the six categories + origin visible (spec section 5):** ALL improvements
  (Lint + generic) are in the six Lint categories. Generic rules are placed in a category for
  ...
  an **origin badge** (Lint / MxCLI / Mxlint.com) — Lint rules first — so that
  calibrated Lint is never confused with engine-generic. The internal
  `Violation.category` remains the engine prefix; this is purely a display mapping.
- **Severity:** Lint rules show their Lint severity (from the registry); generic rules
  show the mxcli engine severity LITERALLY (error/warning/info/hint) — not translated.
  Both as a severity chip. An Lint severity outside the four (`TODO-confirm`) falls under
  "To be confirmed".
- **Summary card (three cards):** counts ALL improvements — per category, per severity, and
  per **origin** (Lint (calibrated) / MxCLI Mxlint / Mxlint.com) so that the distribution
  Lint-vs-generic remains visible.
...
Lint rules their `acrCode` (e.g. CLEVR-HYG-001 → DuplicateEntityNames), generic
...
the ruleId), which is why `LintScanService` fetches the catalog separately and sends it as
...
filter/summary card, e.g. `2185 improvements (77 Lint / 2108 MxCLI Mxlint / 0 Mxlint.com)
— 2185 raw, exit 0` (counts calculated from the violations via `originOf`).

**Engine filter (origin):** below the button there are toggles **Lint / MxCLI Mxlint /
...
| File | Role |
|---|---|
| [`LintScanSettings.cs`](LintScanSettings.cs) | configurable `mxcliPath` + `projectPath` (from `lint-scan-settings.json`) |
| [`LintScanService.cs`](LintScanService.cs) | orchestrates run → parse → normalize → JSON (IO/wiring only, no normalization logic) |
| [`lint-scan-settings.json`](lint-scan-settings.json) | the settings (see below) |
| `rules.json` (build-link to [`../csharp-normalizer/rules.sample.json`](../csharp-normalizer/rules.sample.json)) | the Lint registry, single source of truth |

The normalizer + registry are reused **unchanged** (project reference to
`Clevr.Lint.Normalizer`). The pure parse helpers `MxcliOutputParser` and
`RuleRegistryJson` have been added as **new** files to that library (the
existing, tested classes have not been touched).

### Configuration (not hardcoded)
Fill in [`lint-scan-settings.json`](lint-scan-settings.json) before running:
```json
...
1. `dotnet build -c Debug` (in `csharp-spike`). Output: `bin\Debug\net10.0\` with
   `Clevr.AcrSpike.dll`, **`Clevr.Lint.Normalizer.dll`**, `manifest.json`,
   `rules.json`, `lint-scan-settings.json` and `wwwroot\`.
2. Copy the **entire** contents of `bin\Debug\net10.0\` to
   `<app>\extensions\clevrlintspike\` (the normalizer dll must be included!).
3. Start Studio Pro with `--enable-extension-development`, **F4** to (re)load.
4. **Extensions → … → CLEVR Lint Spike** → click **Scan for improvements**.

### What you expect to see
- A summary line, e.g. `2185 improvements (77 Lint / 2108 generic) — 2185 raw, exit 0`.
- The **Lint layout**: a summary card (per category / per severity / per origin) and the
  six Lint categories with expandable rules per category. Each category contains both
  our verified Lint rules (badge **Lint**, CLEVR ruleId, category/severity from the
...
- **Compiles:** yes — `Clevr.AcrSpike` + `Clevr.Lint.Normalizer` build cleanly (.NET 10).
- **Chain correct:** proven with unit tests (`DataChainTests`): `rules.sample.json` →
  ...
  kind/category/severity (ACR_ENT_ATTRS → lint/Maintainability/Minor; MPR001 →
...
1. Registers a **C#-managed** dockable pane, opened via
   **Extensions → … → CLEVR Lint Spike** (`IDockingWindowService.OpenPane`).
...
2. Create `<app>\extensions\clevrlintspike\`.
...
6. Open the panel via **Extensions → … → CLEVR Lint Spike** and click **Run command**.
