...
///   "RunAcrScan"  → Phase 2A: runs the REAL mxcli engine via AcrScanService,
...
public class SpikeDockablePaneViewModel : WebViewDockablePaneViewModel
{
    ...
    public override void InitWebView(IWebView webView)
    {
        ...
        webView.MessageReceived += (_, args) =>
        {
            if (args.Message == "RunCommand")
            {
                ...
                _logService.Info($"[CLEVR ACR] command done, exit={r.ExitCode}, ok={r.Ok}");
                string report =
                    $"exitCode: {r.ExitCode}\nok: {r.Ok}\n\n" +
                    $"--- stdout ---\n{r.StdOut}\n--- stderr ---\n{r.StdErr}" +
                    (r.Error is null ? "" : $"\n\n--- error ---\n{r.Error}");
            ...
            else if (args.Message == "ExportHtml")
            {
                ...
                try
                {
                    ...
                    _logService.Info($"[CLEVR ACR] report saved: {path}");
                    ReportExporter.TryOpen(path);
                    webView.PostMessage("ReportSaved", path);
                }
                catch (Exception ex)
                {
                    _logService.Error("[CLEVR ACR] report export failed", ex);
    ...
    // $project/.clevr-acr/exclusions.json (included in version control). The render layer filters
    ...
    private void PostExclusions(IWebView webView)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] loading exclusions failed", ex);
    ...
    private void AddExclusion(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] adding exclusion failed", ex);
            webView.PostMessage("ExclusionError", ex.Message);
        }
    }
    ...
    private void AddExclusions(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] adding exclusions (batch) failed", ex);
            webView.PostMessage("ExclusionError", ex.Message);
        }
    }
    ...
    private void RemoveExclusion(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] removing exclusion failed", ex);
            webView.PostMessage("ExclusionError", ex.Message);
        }
    }
    ...
    private void RemoveExclusions(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] removing exclusions (batch) failed", ex);
            webView.PostMessage("ExclusionError", ex.Message);
        }
    }
    ...
    private void PostManualChecks(IWebView webView)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] loading manual checks failed", ex);
    ...
    private void AnswerManualCheck(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] answering manual check failed", ex);
            webView.PostMessage("ManualCheckError", ex.Message);
        }
    }
    ...
    private void ClearManualCheck(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] clearing manual check failed", ex);
            webView.PostMessage("ManualCheckError", ex.Message);
        }
    }

    /// <summary>
    /// The project directory in which .clevr-acr/exclusions.json resides — THE SAME directory
    /// the scan uses (acr-scan-settings.json → projectPath; .mpr → its containing directory;
    ...
    ///   1. via the stable GUID (documentId, populated by mxcli/ACR) → TryGetAbstractUnitById;
    ...
    private void OpenDocument(IWebView webView, JsonObject? data)
    {
        ...
        try
        {
            ...
            if (unit == null)
            {
                ...
                _logService.Info($"[CLEVR ACR] OpenDocument: not found (id='{documentId}', qn='{qualifiedName}', type='{documentType}')");
        ...
        catch (Exception ex)
        {
            ...
            _logService.Error("[CLEVR ACR] OpenDocument failed", ex);
    ...
    private void OpenUrl(IWebView webView, JsonObject? data)
    {
        ...
        {
            _logService.Error("[CLEVR ACR] OpenUrl failed", ex);
    ...
    private void RunFullScan(IWebView webView, SynchronizationContext? uiContext, bool deepScan)
    {
        ...
        _logService.Info($"[CLEVR ACR] full scan started (projectDir='{projectDir}')");
        ...
        _ = Task.Run(() =>
        {
            try
            {
                ...
                // deepscan — the describe findings PER CHUNK (~20-30s each). Each batch goes as "AcrViolations"
                ...
                catch (Exception ex)
                {
                    ...
                    Post("AcrViolations", AcrScanService.ErrorJson($"Unexpected error during mxcli scan: {ex.Message}"));
                }
            ...
            catch (Exception ex)
            {
                ...
                _logService.Error("[CLEVR ACR] full scan failed", ex);
    ...
    private void SafePost(IWebView webView, string? projectDir, string message, string data)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logService.Error($"[CLEVR ACR] PostMessage({message}) failed", ex);
        }
    }
