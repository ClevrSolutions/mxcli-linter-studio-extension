# CLEVR Lint — Project Handover (final state June 2026)

> **How to use:** paste this document into a new conversation with Claude (or hand it to Claude Code) to seamlessly continue work on the CLEVR Lint extension. It describes what the project is, where it stands, how it works, and the working practices that underpin it. Read it alongside **`clevr-lint-shell-status.md`** (the living compass, with all measurement and decision details) and **`clevr-lint-shell-spec.md`** (the data contract).

---

## 1. What is this project?

The **CLEVR Lint** extension is a Mendix Studio Pro 11 extension (an "Lint shell") that displays lint findings ("improvements") in Lint style: six categories (Security, Reliability, Performance, Architecture, Maintainability, Project hygiene), severities Minor < Major < Critical < Blocker, clickable filters, exclusions with reason, manual checks, and an exportable HTML report.

**Core idea:** an *aggregator* of model-readable findings — mxcli's own lint + CLEVR's own calibrated rules + CLEVR context — **not a replacement** for Lint or the live Studio Pro analysis.
...
- **`dist/CLEVR-Lint-extension/`** — the shareable end-user package (installer + README + `clevrlint/` payload). Built by `Build-Package.ps1`.
...
**Division of roles:** chat Claude = sparring partner/prompts. **Claude Code** (on `C:\Apps\clevr-lint-shell`) = builds & verifies. Michel tests in Studio Pro and pastes results back.

**Deploy:** no more manual copying. `Build-Package.ps1` builds + assembles `dist/CLEVR-Lint-extension(.zip)`; `Install-ClevrLint.ps1` installs into `<project>\extensions\clevrlint` and **automatically fetches mxcli** (GitHub release, sha256-verified, after a single Y/n confirmation). Then restart Studio Pro; fixes apply to NEW scans.
...
- **External binary:** `mxcli.exe` (v0.12.0, **Apache-2.0**) — **auto-downloaded** by the installer from the official GitHub release, **sha256 + byte size verified** before use, stored in `%LOCALAPPDATA%\clevr-lint\mxcli\`. **No more mxlint** (the previous supply-chain concern about the auto-downloaded mxlint binary is no longer applicable).
