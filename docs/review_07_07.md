...
- **`TestHarness/Program.cs` (789 LOC) duplicates the extension's message router.** Its ~350-line `DispatchMessage` switch re-implements the same message→coordinator wiring as `DockablePaneViewModel`. Every new message type must be added twice; a miss makes dev mode silently diverge from Studio Pro (several already have — see §6).
- **The WebView shim exists twice** — as a C# raw string (`Program.cs:713-759`) and a TS template literal (`vite.config.ts:12-46`), kept in sync by comment only. Fix a bug in one and the other dev path keeps it.
...
- **U7 — All four dialogs lack Escape-to-close, focus trapping, and `role="dialog"`/`aria-modal`**; and interactive `div`/`tr`/`h3` elements throughout (`ViolationInstance.tsx:52-56`, `SummaryCards.tsx:47-131`, `Settings.tsx:184,258`) have no keyboard path at all — keyboard users cannot open documents or toggle filters.
...
- **X3 (medium) — Harness has no handlers for `CancelScan` or `BrowseMxcliPath`** — both buttons are silent no-ops in every dev session, so regressions in those flows are invisible until Studio Pro testing.
- **X4 (low-med) — Harness `Modules` payload uses the legacy `string[]` shape** (`Program.cs:536`), which is the *only* reason the back-compat branch in `useMessageBus.ts:73-77` exists; the marketplace badge and "Exclude all marketplace" bulk action can never be exercised in dev. Harness `RulesCatalog` also omits `ruleDescriptions`/`ruleStarContent`, so the RuleInfoDialog content path is untestable.
...
The root cause of X1–X4 is structural: the harness re-implements the message router by hand (§2). A shared router consumed by both `DockablePaneViewModel` and the harness would make this class of drift impossible.
...
- **S5 — `Pack-Dist.ps1:41-49` hardcodes the artifact list** (`YamlDotNet.dll` by name). Adding a NuGet dependency and forgetting this list ships a dist that throws `FileNotFoundException` in Studio Pro — and the stale-file sweep would actively delete a manually-added DLL. Copy `*.dll`/`*.deps.json` from the release output (minus the Mendix API) instead. Also: the `wwwroot` copy is a merge, not a sync — removed/renamed assets linger in dist forever (`:55`).
- **S6 — Hardcoded port 5174 with unguarded `listener.Start()`** (`Program.cs:134,157-159`): an orphaned harness (concurrently on Windows doesn't reliably kill the `dotnet run` grandchild) crashes the new one with a raw stack trace while Vite keeps proxying to nothing.
...
12. **Fix harness fidelity**: ~~real `appStoreModules` decision (X1)~~ ✅ done (mock aligned to production's empty array); still open: `CancelScan`/`BrowseMxcliPath` handlers, modern `Modules` shape, then delete the UI's back-compat branch (X3, X4).
